...
export class MilesLeaderboardComponent implements OnInit {
	...
	formGroup: FormGroup = this.fb.group({
		selectedSeason: this.fb.control(
			this.activatedRoute.snapshot.queryParamMap.has("t")
				? (this.toSeason(this.activatedRoute.snapshot.queryParamMap.get("t")))
				: "Gesamt"
		)
	});
	...
	private toSeason(queryParamValue: string): string {
		let season = "Gesamt";
		const seasonWasSet = seasonOptions.some(option => {
			if (queryParamValue === option) {
				season = option;
				return true;
			}
	...
	constructor(private userService: UserService,
		...
		this.subscriptions.push(
			this.activatedRoute.queryParamMap
				.pipe(filter(it => it.has("t")))
				.subscribe(queryParamMap => {
					this.formGroup.get("selectedSeason").setValue(this.toSeason(queryParamMap.get("t")))
				}),
			this.formGroup.get("selectedSeason").valueChanges.pipe(startWith(this.formGroup.get("selectedSeason").value),)
				.subscribe(season => {
			...
			this.users$
				.pipe(
					//sort by miles
					switchMap(users => this.addMiles(users)),
	...
	private addMiles(users: User[]): Observable<any[]> {
		return this.formGroup.get("selectedSeason").valueChanges.pipe(
			startWith(this.formGroup.get("selectedSeason").value),
			tap(() => this.leaderBoard$.next(null)),
			switchMap(season => this.getMiles$(season)),
			map(milesList => [
				...users.map(user => {
